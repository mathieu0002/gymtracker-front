...
import type { MuscleGroupDto, ExerciseDto, SplitType } from '../types'
...
const inputStyle: React.CSSProperties = {
  ...
  color: 'var(--text)', fontSize: 13, fontFamily: 'Exo 2, sans-serif',
...
const btnPrimary: React.CSSProperties = {
  ...
  color: 'var(--orange)', fontSize: 12, fontFamily: 'Rajdhani, sans-serif',
...
const btnDelete: React.CSSProperties = {
  ...
  border: '1px solid #2a1010', background: 'none',
  color: 'var(--red)', fontSize: 11, fontFamily: 'Rajdhani, sans-serif',
...
export default function Admin() {
  ...
  useEffect(() => {
    Promise.all([getMuscleGroups(), getExercises()])
      .then(([mgs, exs]) => {
        setMuscleGroups(mgs)
        setExercises(exs)
  ...
  const handleAddMuscleGroup = async () => {
    ...
    try {
      ...
      setMgError('Erreur — ce nom existe peut-être déjà')
    } finally { setMgLoading(false) }
  }

  const handleDeleteMuscleGroup = async (id: string) => {
    if (!confirm('Supprimer ce groupe ? Les exercices liés seront aussi supprimés.')) return
  ...
  const handleAddExercise = async () => {
    ...
    try {
      const ex = await createCustomExercise({
        name: exName.trim(),
        muscleGroupId: exMgId,
        description: exDesc.trim() || undefined,
      })
      setExercises(prev => [...prev, ex])
      setExName(''); setExDesc('')
    } catch {
      setExError('Erreur — ce nom existe peut-être déjà')
  ...
  const filteredExercises = filterSplit === 'all'
    ? exercises
    : exercises.filter(ex => {
        const mg = muscleGroups.find(mg => mg.id === ex.muscleGroupId)
        return mg?.split === filterSplit
      })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 48 }} className="fade-in">
      ...
      <div>
        ...
        <div style={{ fontFamily: 'Share Tech Mono', fontSize: 11, color: 'var(--text-dim)', marginTop: 4, letterSpacing: 1 }}>
          DATABASE_MANAGEMENT // EXERCISES & MUSCLE_GROUPS
        </div>
      </div>
      ...
      <section>
        ...
        <div style={{
          ...
          borderTop: '2px solid var(--orange)', borderRadius: 8,
          padding: 20, marginBottom: 16,
        }}>
          <div style={{ fontFamily: 'Share Tech Mono', fontSize: 11, color: 'var(--text-dim)', marginBottom: 14, letterSpacing: 1 }}>
            // NOUVEAU GROUPE
          </div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const, alignItems: 'flex-end' }}>
            <div style={{ flex: 1, minWidth: 180 }}>
              ...
              <input
                value={mgName} onChange={e => setMgName(e.target.value)}
            ...
            <div style={{ minWidth: 160 }}>
              ...
              <select
                value={mgSplit} onChange={e => setMgSplit(e.target.value as SplitType)}
                style={{ ...inputStyle, width: 'auto', cursor: 'pointer', colorScheme: 'dark' }}
              >
                {SPLITS.map(s => <option key={s} value={s}>{SPLIT_LABELS[s]}</option>)}
              </select>
            </div>
            <button onClick={handleAddMuscleGroup} disabled={mgLoading} style={btnPrimary}>
              {mgLoading ? '...' : '+ AJOUTER'}
            </button>
          </div>
          ...
          {muscleGroups.map(mg => (
            <div key={mg.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{
                  fontFamily: 'Share Tech Mono', fontSize: 10, color: 'var(--orange)',
                  border: '1px solid var(--orange-dim)', borderRadius: 3,
                  padding: '2px 8px', background: 'var(--orange-dim)',
                }}>
      ...
      <section>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const, alignItems: 'flex-end' }}>
            ...
            <div style={{ minWidth: 200 }}>
              ...
              <select
                value={exMgId} onChange={e => setExMgId(e.target.value)}
                style={{ ...inputStyle, cursor: 'pointer', colorScheme: 'dark' }}
              >
                {muscleGroups.map(mg => (
                  <option key={mg.id} value={mg.id}>
                    {mg.name} ({SPLIT_LABELS[mg.split as SplitType] ?? mg.split})
                  </option>
                ))}
              </select>
            </div>
            ...
            <button onClick={handleAddExercise} disabled={exLoading} style={btnPrimary}>
              {exLoading ? '...' : '+ AJOUTER'}
            </button>
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
          <button
            ...
            style={{
              padding: '5px 12px', borderRadius: 4, border: `1px solid ${filterSplit === 'all' ? 'var(--orange)' : 'var(--border)'}`,
              background: filterSplit === 'all' ? 'var(--orange-dim)' : 'transparent',
              color: filterSplit === 'all' ? 'var(--orange)' : 'var(--text-dim)',
          ...
          {SPLITS.map(s => (
            <button key={s} onClick={() => setFilterSplit(s)} style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {filteredExercises.length === 0 ? (
            <div style={{ fontFamily: 'Share Tech Mono', fontSize: 12, color: 'var(--text-dim)', padding: 20 }}>
              // AUCUN EXERCICE
            </div>
            ...
            return (
              <div key={ex.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  <div style={{
                    fontFamily: 'Share Tech Mono', fontSize: 10, color: 'var(--text-dim)',
                    border: '1px solid var(--border)', borderRadius: 3,
                    padding: '2px 8px', minWidth: 90, textAlign: 'center' as const,
                  }}>
                  ...
                  {ex.isCustom && (
                    <div style={{
                      fontFamily: 'Share Tech Mono', fontSize: 9, color: 'var(--orange)',
                      border: '1px solid var(--orange-dim)', borderRadius: 3,
                      padding: '2px 6px', background: 'var(--orange-dim)',
                    }}>
                      CUSTOM
                    </div>
                  )}
