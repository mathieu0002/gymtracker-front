    ...
    <div className="fade-in">
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ fontFamily: 'Share Tech Mono', fontSize: 11, color: 'var(--text-dim)', marginTop: 4, letterSpacing: 1 }}>
          {currentUser?.firstName.toUpperCase()} // SESSION_LOG // {sessions.length} ENTRÉE(S)
        </div>
      </div>
      ...
      {sessions.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {sessions.map((session, idx) => (
            <div key={session.id} style={{ background: 'var(--bg-card)', borderRadius: 8, border: '1px solid var(--border)', overflow: 'hidden' }}>
              <div onClick={() => setExpanded(expanded === session.id ? null : session.id)} style={{
                padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                cursor: 'pointer', borderLeft: `3px solid ${expanded === session.id ? 'var(--cyan)' : 'transparent'}`, transition: 'border-color 0.2s',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ fontFamily: 'Share Tech Mono', fontSize: 10, color: 'var(--text-dim)', minWidth: 32 }}>#{String(idx + 1).padStart(3, '0')}</div>
                  <div>
                    <div style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 15, letterSpacing: 1, color: expanded === session.id ? 'var(--cyan)' : 'var(--text)', textTransform: 'uppercase' as const }}>
                    ...
                    <div style={{ fontFamily: 'Share Tech Mono', fontSize: 11, color: 'var(--text-dim)', marginTop: 3 }}>
                      {new Date(session.sessionDate).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase()}
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  >SUPPR</button>
                  <span style={{ color: 'var(--text-dim)', fontSize: 12, fontFamily: 'Share Tech Mono' }}>{expanded === session.id ? '▲' : '▼'}</span>
                </div>
              </div>
              {expanded === session.id && session.exercises && (
                <div style={{ borderTop: '1px solid var(--border)', padding: '16px 20px 20px 20px' }}>
                  {session.exercises.length === 0 ? (
                    <div style={{ fontFamily: 'Share Tech Mono', fontSize: 12, color: 'var(--text-dim)' }}>// AUCUN EXERCICE</div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      {session.exercises.map(ex => (
                        <div key={ex.id}>
                          <div style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 600, fontSize: 14, letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase' as const, display: 'flex', alignItems: 'center', gap: 10 }}>
                            {ex.exerciseName}
                            <span style={{ fontFamily: 'Share Tech Mono', fontSize: 10, color: 'var(--text-dim)', fontWeight: 400, textTransform: 'none' as const }}>{ex.muscleGroupName}</span>
                          </div>
                          {ex.sets.length === 0 ? (
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {ex.sets.map((set, i) => (
                                <div key={set.id} style={{ background: 'var(--bg)', border: '1px solid var(--border-bright)', borderRadius: 4, padding: '5px 12px', fontFamily: 'Share Tech Mono', fontSize: 12, color: 'var(--text-mid)', display: 'flex', alignItems: 'center', gap: 6 }}>
